...
def xywh2xyxy(x):
    '''Convert nx4 boxes (Torch) from [x, y, w, h] to [x1, y1, x2, y2]
...
def predictOnFrame(frame,model):
    ''' return the model preds for the frame'''
    ...
    preds = pred[0]
    clsid = preds.cpu().detach().numpy()[:,-1]
    preds = preds.cpu().detach().numpy()[:,:-1] # xyxy format + conf_score
    return(preds,clsid)
